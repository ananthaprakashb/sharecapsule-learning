...
async function hashText(value){const digest=await crypto.subtle.digest('SHA-256',new TextEncoder().encode(value));return [...new Uint8Array(digest)].map((b)=>b.toString(16).padStart(2,'0')).join('')}
async function searchCached(query,env,{count=8,force=false}={}){
  const providers=configuredProviders(env),cacheKey=await hashText(JSON.stringify({version:2,query,count,providers})),cacheRequest=new Request(`https://search-cache.internal/v2/${cacheKey}`,{method:'GET'}),cache=caches.default
  if(!force){const hit=await cache.match(cacheRequest);if(hit)return{...await hit.json(),cacheHit:true}}
  const batch=await searchWithFallback(query,env,{count}),cacheable=new Response(JSON.stringify(batch),{headers:{'Content-Type':'application/json','Cache-Control':`public, max-age=${QUERY_CACHE_SECONDS}`}});await cache.put(cacheRequest,cacheable);return{...batch,cacheHit:false}
}
export async function researchTarget(body,env,{force=false}={}){
  validateResearchRequest(body);const request=sanitizeResearchRequest(body),queries=buildQueries(request)
  const batches=await Promise.all(queries.map(async(query)=>{const batch=await searchCached(query,env,{count:8,force});return{query,provider:batch.provider,cacheHit:batch.cacheHit,results:batch.results.map((result)=>({...result,query,provider:batch.provider,queryCacheHit:batch.cacheHit}))}}))
  const rawSources=batches.flatMap((batch)=>batch.results),providerByUrl=new Map(rawSources.map((source)=>[source.url,source.provider])),queryCacheByUrl=new Map(rawSources.map((source)=>[source.url,source.queryCacheHit]))
