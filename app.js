import { buildDiagnostic, buildGapList, buildPlan, buildReadiness, scoreDiagnostic } from './engine.js'
...
const state = {
  profile: loadProfile(), stage: 'intake', questions: [], answers: {}, questionIndex: 0, result: null,
}
...
function shell(content, progressStep = 0) {
  return `<div class="app-shell">
    <header class="site-header"><a class="brand" href="#" data-action="home"><span class="brand-mark">S</span><span><b>shareCapsule</b><small>PREPARE</small></span></a><div class="header-note">Diagnostic-first learning</div></header>
    <main>
      <section class="hero"><div class="hero-copy"><div class="eyebrow">Prepare for the goal, not just the syllabus.</div><h1>Know the gap.<br><em>Master what matters.</em></h1><p>Tell us where you need to be and when. We’ll measure where you are today and turn the difference into a focused preparation plan.</p></div>
      <div class="hero-proof"><span>01</span><p><strong>Diagnose first.</strong> No generic course dump.</p><span>02</span><p><strong>Plan to the date.</strong> Available time changes the strategy.</p><span>03</span><p><strong>Explain the gap.</strong> See why each skill gets priority.</p></div></section>
      ${progress(progressStep)}
      ${content}
    </main>
    <footer><span>ShareCapsule Prepare · Phase 1 MVP</span><span>Preparation guidance, not an outcome guarantee.</span></footer>
  </div>`
}
...
function renderIntake() {
  ...
  const trackFields = p.track === 'academic'
    ? `<div class="form-grid">
      ...
      ${field('Topics to cover', textarea('topics', p.topics, 3, 'Quadratics, logarithms, sequences…'), 'Comma-separated is fine.')}
    </div>`
    : p.track === 'interview'
      ? `<div class="form-grid">
        ...
        ${field('Job description / known interview focus', textarea('jobDescription', p.jobDescription, 4, 'Paste the important parts of the job description…'), 'Phase 1 uses this to select a representative diagnostic. AI + source-backed company research comes next.')}
      </div>` : ''
  ...
  app.innerHTML = shell(`<section class="panel intake-panel">
    ...
    <p class="lede">We start with the outcome and deadline, then test your current condition before recommending what to study.</p>
    ...
    </div>${trackFields}<div class="privacy-note">Phase 1 runs entirely in your browser. No account is required and this prototype does not upload these answers.</div>
    <button class="primary-button" type="submit">Start my diagnostic <span>→</span></button></form>` : ''}
  </section>`, 0)
  bindCommon()
  ...
  if (form) {
    ...
    form.addEventListener('submit', (event) => {
      ...
      saveProfile(); state.questions = buildDiagnostic(state.profile, 8); state.answers = {}; state.questionIndex = 0; state.stage = 'diagnostic'; renderDiagnostic(); window.scrollTo({ top: 0, behavior: 'smooth' })
    })
  }
}
...
function renderDiagnostic() {
  ...
  app.innerHTML = shell(`<section class="panel diagnostic-panel">
    <div class="diagnostic-head"><div><div class="eyebrow">Diagnostic · ${state.questionIndex + 1} of ${state.questions.length}</div><h2>${escapeHtml(q.competency)}</h2></div><button class="text-button" data-action="edit-goal">Edit goal</button></div>
    <div class="meter"><span style="width:${((state.questionIndex + 1) / state.questions.length) * 100}%"></span></div>
    <div class="question-card"><div class="question-meta"><span>${escapeHtml(q.difficulty)}</span><span>${escapeHtml(q.competency)}</span></div><h3>${escapeHtml(q.prompt)}</h3>
    ...
    <div class="diagnostic-actions"><small>Choose the best answer. We use the result to prioritize gaps.</small><button class="primary-button compact" data-action="next" ${selected === undefined ? 'disabled' : ''}>${state.questionIndex === state.questions.length - 1 ? 'See my gap analysis' : 'Next question'} <span>→</span></button></div>
  </section>`, 1)
  bindCommon()
  document.querySelector('[data-action="edit-goal"]').addEventListener('click', renderIntake)
  document.querySelectorAll('[data-option]').forEach((button) => button.addEventListener('click', () => { state.answers[q.id] = Number(button.dataset.option); renderDiagnostic() }))
  document.querySelector('[data-action="next"]').addEventListener('click', () => {
    if (state.answers[q.id] === undefined) return
    if (state.questionIndex < state.questions.length - 1) { state.questionIndex += 1; renderDiagnostic() }
    else { state.result = scoreDiagnostic(state.questions, state.answers); state.stage = 'results'; renderResults(); window.scrollTo({ top: 0, behavior: 'smooth' }) }
  })
}
...
function renderResults() {
  ...
  app.innerHTML = shell(`<section class="results-wrap">
    <div class="results-hero panel"><div><div class="eyebrow">Initial readiness estimate</div><h2>${readiness.readiness}%</h2><strong>${readiness.label}</strong><p>${readiness.days} day${readiness.days === 1 ? '' : 's'} until your target date. This is a preparation indicator based on this short diagnostic, not a prediction of an exam or hiring outcome.</p></div>
    <div class="score-ring" style="--score:${readiness.readiness * 3.6}deg"><div><b>${result.correct}/${result.total}</b><span>diagnostic</span></div></div></div>
    <div class="results-grid"><div class="panel result-panel"><div class="eyebrow">Measured condition</div><h3>Where to focus first</h3><div class="skill-list">
      ${gaps.map((gap) => `<div class="skill-row"><div><strong>${escapeHtml(gap.name)}</strong><span>${gap.score}% measured</span></div><div class="skill-bar"><span style="width:${Math.max(gap.score, 4)}%"></span></div><b class="${gap.score < 60 ? 'risk' : 'ok'}">${gap.score < 60 ? 'Priority gap' : gap.score < 80 ? 'Build' : 'Maintain'}</b></div>`).join('')}
    ...
    <div class="panel next-panel"><div><div class="eyebrow">Phase 1 boundary</div><h3>Next: personalized resources and re-testing</h3><p>This MVP proves the diagnostic → gap → deadline plan loop. The next coding pass will add persisted learner state, AI-generated target-specific questions, source-backed videos/documents, and micro-assessments.</p></div><button class="secondary-button" data-action="restart">Prepare another goal</button></div>
  </section>`, 3)
  bindCommon()
